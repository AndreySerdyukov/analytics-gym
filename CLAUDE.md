...
Поле `topic` конспекта — не украшение: по нему строится боковой список раздела теории. Конспект
без темы попадает в группу «Без темы» в конце, туда же — конспект темы, убранной из `blocks.yaml`
(`sync` архивирует тему, а не удаляет).
